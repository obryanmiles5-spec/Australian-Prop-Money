...
for (const [id, data] of Object.entries(imgMap)) {
  ...
  if (match) {
    code = code.substring(0, match.index) + 
           match[1] + "'" + data.image + "'" + match[2] + 
           JSON.stringify(data.gallery).replace(/"/g, "'") + 
           code.substring(match.index + match[0].length);
  } else {
    const reg2 = new RegExp(`(id:\\s*'${id}',[\\s\\S]*?image:\\s*)'[^']*'`);
    let match2 = reg2.exec(code);
    if (match2) {
      code = code.substring(0, match2.index) + 
             match2[1] + "'" + data.image + "'" + 
             code.substring(match2.index + match2[0].length);
    }
  }
}

// Ensure the category renaming is kept!
code = code.replace(/wholesale-packs/g, 'bundle-packs');
code = code.replace(/Wholesale Packs/g, 'Bundle Packs');
